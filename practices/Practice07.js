...
const leastQuantity = appleStore.reduce((leastCounted, curr) => leastCounted.quantity < 
curr.quantity ? leastCounted: curr, {});
...
// Find all the items in the cart with their quantity -> [ 'Laptop, 1', 'Smartphone 2', 'Headphones, 3' ]
const itemsWithQuantities = shoppingCart.items.map(item => `${item.productName}, $
{item.quantity}`);
